...
export default function AdesaoClienteBeta({ token }) {
  ...
  return <main className="min-h-dvh bg-creme-50 px-6 py-16 text-grafite-900">
    ...
    <div className="mx-auto max-w-xl space-y-6">
      ...
      {estado === 'concluido' ? <p role="status">Adesão registada. A tua inscrição original foi preservada. Os benefícios começam na abertura da fase beta da tua unidade e mantêm-se enquanto o 100PRESSÃO estiver em atividade.</p> : <form onSubmit={aderir} className="space-y-5">
        ...
        <p>A partir da abertura da fase beta da tua unidade: 10% no consumo próprio e acesso à ementa exclusiva, nos termos do regulamento. A inauguração fecha as inscrições, não o teu desconto.</p>
        <label className="flex items-start gap-3"><input type="checkbox" checked={dados.aceita_regulamento} onChange={(e) => setDados({ ...dados, aceita_regulamento: e.target.checked })} className="mt-1 h-5 w-5 shrink-0" /><span>Aceito o <a href="/legal/cliente-beta/regulamento-2026-09-16.v1.txt" target="_blank" rel="noreferrer" className="underline">regulamento Cliente Beta</a>.</span></label>
        <label className="flex items-start gap-3"><input type="checkbox" checked={dados.aviso_lido} onChange={(e) => setDados({ ...dados, aviso_lido: e.target.checked })} className="mt-1 h-5 w-5 shrink-0" /><span>Li o <a href="/legal/cliente-beta/aviso-2026-09-16.v1.txt" target="_blank" rel="noreferrer" className="underline">aviso de privacidade</a>.</span></label>
